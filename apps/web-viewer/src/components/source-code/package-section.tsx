...
import type { PackageSources } from "@/lib/source-files";
...
export function PackageSection({ pkg }: { pkg: PackageSources }) {
  return (
    <section className="mt-8">
      <h2 className="font-mono text-base font-semibold md:text-lg">
        {pkg.name}
      </h2>
      <SourceCode files={pkg.files} entryPath={pkg.entryPath} />
    </section>
  );
}
